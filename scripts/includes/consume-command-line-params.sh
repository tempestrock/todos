#
# Handling of command line arguments and printing usage information for the 'deploy' script.
#
...
print_usage() {
  ...
  echo
  echo "Deploys the latest ToDos app."
  echo
  echo "Options:"
  echo "  -e, --env-name [uat|prod]         set the environment's name"
  echo "  -h, --help                        display this usage help (optional)"
  echo
  echo "Example:"
  echo "  ${script_name} --env-name uat          deploys environment 'uat'"
  echo
}
...
consume_command_line_params() {
  ...
  *)
    print_usage_and_exit "Values for option [-e|--env-name] may only be 'uat' or 'prod'."
    ;;
  esac
}
